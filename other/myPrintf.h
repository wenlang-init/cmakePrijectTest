...
#define MY_PRINTF_INFO() MY_PRINTF(GREEN "%s:%d %s" RED "---" RESET,__FUNCTION__,__LINE__,__FILE__)

#define MY_PRINTF_BUFFER(buffer,bufferSize,__format, ...) printFormatBuffer(buffer, bufferSize, __format, ## __VA_ARGS__)
#define MY_PRINTF_BUFFER_INFO(buffer,bufferSize) \
    MY_PRINTF_BUFFER(buffer,bufferSize,GREEN "%s:%d %s" RED "---" RESET,__FUNCTION__,__LINE__,__FILE__)
...
do { \
    ...
    MY_PRINTF_BUFFER(buffer + len,bufferSize - len,__format, ## __VA_ARGS__); \
} while(0)
...
do { \
    ...
    MY_PRINTF_BUFFER(printf_info + len,PRINT_BUFFER_SIZE - len,__format, ## __VA_ARGS__);\
} while(0)

// #define MY_PRINTF_DEBUG(__format, ...) \
// do { \
//     MY_PRINTF(GREEN "%s:%d %s" RED "---" RESET,__FUNCTION__,__LINE__,__FILE__); \
