...
#ifndef TO__FILENAME__
// 将filename转为文件名,并存储到dstname
#define TO__FILENAME__(dstname,filename) \
    do{\
        if(filename){\
            memcpy(dstname,filename,strlen(filename)+1);\
            char *ptr = dstname;int i = 0;\
            while(ptr[i]){if(ptr[i] == '\\')ptr[i]='/';i++;}\
            char *p = strrchr(dstname, '/');\
            if(p){\
                p++;i=0;\
                while(p[i]){dstname[i]=p[i];i++;}\
                dstname[i]=0;\
            }\
        } else {\
            dstname[0] = 'n';dstname[1] = 'u';\
            dstname[2] = 'l';dstname[3] = 'l';dstname[4] = 0;\
        }\
    }while(0)
#endif
...
#endif
// 获取工作路径
#define GET_CURRENTPATH(buff,len) getcwd((char*)buff, (int)len)
