...
void printDebugMsg(LOG_TYPE_ENUM type,const char* function,const char *file,const int line,const char* format, ...)
{
    ...
    if(size >= 4096){
        ...
        va_end (arg);
        fprintf(out,"%s%s:" GREEN "time:%s(%.6lf)" BOLDBLACK "|" RESET BOLDYELLOW "PId:%lu" BOLDBLUE "Tid:%lu" BOLDBLACK "|" RESET CYAN "%s:%d" BLUE "(%s)" MAGENTA "---" RESET "%s",colorHead,typemsg,__get_printfTime(cur_time,sizeof(cur_time)),__get_printfTime_d(), ProcessId,ThreadId,file, line,function,buffer);
        free(buffer);
    } else {
        char buffer[4096];
        va_start (arg, format);
